...
- **`bin/.local/bin/vault`** — thin CLI all the Claude pieces call so they stay declarative and share one source of truth for the layout + git-sync: `dir` (print vault path, error if absent), `slug` (derive project key from cwd's git remote, fallback dir name), `moc <slug>` (ensure project MOC, idempotent), `reindex` (regenerate every MOC's link lists + the top index from the notes on disk), `sync "<msg>"` (reindex, then add+commit+push, push failure non-fatal), `delete <path>` (delete a note by path or name, then sync).
